...
function compressionJSON(filename) {
...
const compressionJSONs = ['entities', 'items', 'cameras/presets'].map(name => {
  const path = SRC_PATH + name;
  return fs.readdirSync(path).map(filename => compressionJSON(path + '/' + filename));
}).flat(1);
const compressionManifestJSON = compressionJSON(SRC_PATH + 'manifest.json');

const archive = archiver('zip', {
  zlib: { level: 9 }
});

archive.append(compressionManifestJSON[1], { name: 'manifest.json' });
archive.file(SRC_PATH + 'pack_icon.png', { name: 'pack_icon.png' });
compressionJSONs.forEach(data => archive.append(data[1], { name: data[0].substring(SRC_PATH.length) }));
